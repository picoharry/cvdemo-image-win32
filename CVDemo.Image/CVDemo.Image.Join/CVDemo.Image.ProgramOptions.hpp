	...
	namespace image
	{
		...
		public:
			...
			void DisplayInputOptions(std::ostream& os)
			{
				...
				if (vm.count("foreground-image-file")) {
					fgFiles = vm["foreground-image-file"].as<std::vector<std::string>>();
				}
				for (auto &i : fgFiles) {
				...
				if (vm.count("image-join-orientation")) {
					os << "Image join orientation: " << vm["image-join-orientation"].as<std::string>() << std::endl;
				}
		...
		public:
			...
			void DisplayImageJoinOrientation(std::ostream& os)
			{
				if (vm.count("image-join-orientation")) {
					os << "Image join orientation: " << vm["image-join-orientation"].as<std::string>() << std::endl;
				}
		...
		public:
			...
			std::vector<std::string> GetForegroundImageFiles()
			{
				...
				if (vm.count("foreground-image-file") > 0) {
					files = vm["foreground-image-file"].as<std::vector<std::string>>();
				}
				return files;
			}
			...
			// TBD: Just use boolean (e.g., isVertical) for orientatin?
			std::string GetImageJoinOrientation()
			{
				if (!isParsed) {
					_Parse();
				}
				...
				if (vm.count("image-join-orientation") > 0) {
					auto v = vm["image-join-orientation"].as<std::string>();
